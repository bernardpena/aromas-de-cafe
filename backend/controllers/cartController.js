  ...
  try {
      if (invitado) {
          ...
          for (const item of items) {
              ...
              console.log("Insertando compra de invitado:", { invitadoId, producto, cantidad, valor });
              await pool.query(
          ...
          for (const item of items) {
              const { producto_id, cantidad, email, descripcion, imagen, nombre, precio } = item; // Asegúrate de que 'precio' esté aquí también
              const valor = precio * cantidad; // Cálculo de el valor

              console.log("Insertando compra de usuario:", { usuario_id, producto_id, cantidad, email, descripcion, imagen, nombre, valor });
              
              await pool.query(
                  "INSERT INTO compras (usuario_id, producto_id, cantidad, email, descripcion, imagen, nombre, valor) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
                  [usuario_id, producto_id, cantidad, email, descripcion, imagen, nombre, valor] // Incluyendo 'valor'
              );
          }
      } else {
